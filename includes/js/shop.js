...
// FUNCTION FOR FETCH NAME AND IMAGES
    function identification(){
        $.ajax({
            url	:	"/ZC/includes/php/function.php",
            method:	"POST",
...
// FUNCTION FOR SEARCH FOOD
    $("#search_btn").click(function(e){
        ...
        if(keyword != ""){
            $("#get_product").html("<div class='alert bg-light text-center' style='color:#9457eb;' role='alert'><h5>Loading...</h5></div>");
            $.ajax({
            url		:	"/ZC/includes/php/function.php",
            method	:	"POST",
...
// FUNCTION FOR FETCH DATA FOR UPDATE MODAL
    function updateProfile(id){
        ...
        $.ajax({
            url: '/ZC/admin/fetchdata/updateCustomer.php/',
            type: 'POST',
            dataType: 'json',
            data: {customerID: id},
        })
        .done(function(response) {
            ...
            $('#updateImage').attr("src","/ZC/admin/assets/customersPhoto/"+response[0].customerImage)
...
// FUNCTION FOR UPDATE PROFILE
    $('#updateBtn').click(function(e){
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "/ZC/includes/php/manageProfile.php",
                method: "POST",
                dataType: "text",
                data:data,
                cache: false,
...
// CHECHOUT 
                    ...
                    success:function(response){
                        if(response == 0){
                            ...
                            if (result.isConfirmed) {
                                ...
                                if($('.paymentOption:checked').val() != 'Cash On Delivery'){
                                    let amount = $('#total_amount').html();
                                    location.href = `./gcash.php?amount=${amount}`;
                                }
                            }
                            })                               
                        }else if(response == 2){
    ...
    // COUPON
    $("body").delegate("#couponBtn","click",function(){
        var code = $("#couponInput").val();
        var total_amount = $('#total_amount').attr('data-amount');

        if(code.length != 0){
            $.ajax({
                url: '/ZC/includes/php/computeDiscount.php',
                method: 'POST',
                data: {computeDiscount: code, totalAmount: total_amount},
                success : function(data) {
...
$('#updateButton').click(function(e){
    ...
    if (result.isConfirmed) {
        ...
        var data = new FormData(currentForm);
        $.ajax({
            url: "/ZC/includes/php/updateAccount.php",
            method: "POST",
            dataType: "text",
            data:data,
            cache: false,
