...
// CANCEL
	if(isset($_POST["getCancel"])){
		error_reporting(0);
		$Cancel = 'Cancel';
		$product_query = "SELECT a.order_id, a.user_id, a.total_amount, a.payment_option, a.order_status, b.order_id, b.product, b.quantity, b.total, b.date_time_bought, c.customerID, c.customerName , c.customerContact, c.customerAddress, d.closetID , d.closetName, d.closetDescription FROM order_manager a , user_orders b , customers c , closet d
		WHERE a.order_status = '$Cancel' AND a.order_id = b.order_id AND a.user_id = c.customerID AND b.product = d.closetID";
		$run_query = mysqli_query($con,$product_query);
		if(mysqli_num_rows($run_query) > 0){
			while($row = mysqli_fetch_array($run_query)){
				$n++;	
